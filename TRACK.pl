...
print AUTOCONF confstr(ROTATE);
print AUTOCONF confstr(SHOWPOINTS);
print AUTOCONF confstr(SHOWNUMBERS);
print AUTOCONF confstr(LINEWIDTH);
...
if ($userName eq "lulu" || $userName eq "dev" || $userName eq "dev18" || $userName eq "render")	#
  {
  $GMIC="/usr/bin/gmic";
  $TRACK="/shared/foss-18/CppMT/build/cmt_track";
  }
...
$seqcenter=$FSTART + (($FEND - $FSTART)/2);
print "pivot frame ($seqcenter) : ";
$pivot = <STDIN>;
chomp $pivot;
...
if (-e "$OOUTDIR") {verbose("$OOUTDIR already exists");}
  ...
  #-----------------------------#
  #do your shit here
  $cmd="$TRACK $IIN $OOUT $EXT $trackid $FSTART $FEND $pivot $ROTATE $SIZE $SHOWPOINTS $SHOWNUMBERS $LINEWIDTH";
  verbose($cmd);
  system $cmd;
  #-----------------------------#
