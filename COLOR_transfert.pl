...
if (($METHOD == 3) || ($METHOD == 0))
    ...
    if (-e $OOUT && !$FORCE)
        ...
        $INCONTENT="$WORKDIR/content.$EXT";
        #
        $cmd="python $LINEARCOLORTRANSFERT --mode $LCTMODE --target_image $INCONTENT --source_image $INSTYLE --output_image $OOUT $LOG2";
        verbose($cmd);
...
if (($METHOD == 4) || ($METHOD == 0))
    ...
    if (-e $OOUT && !$FORCE)
        ...
        $INCONTENT="$WORKDIR/content.$EXT";
        #
        $cmd="$IDEEPCOLOR $INCONTENT $INSTYLE $OOUT $PROTOTXT $CAFFEMODEL $GLOBPROTOTXT $GLOBCAFFEMODEL 2> /var/tmp/colortransfert.log";
        verbose($cmd);
        print("--------> ideepcolor]\n");
        #-----------------------------#
        ($s1,$m1,$h1)=localtime(time);
