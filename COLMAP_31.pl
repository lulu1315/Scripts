...
if ($userName eq "luluf")	#a Paris
  {
  $COLMAPBIN="~/colmap3.3/build/src/exe";
...
if ($userName eq "dev")	#a Paris
  {
  $COLMAPBIN="/shared/foss/colmap-3.1/build/src/exe";
  $TEXRECON="/shared/foss/mvs-texturing-write-obj-groups/build/apps/texrecon/texrecon";
...
#
$cmd="$COLMAPBIN/feature_extractor --database_path $CWD/$COLMAPDIR/sift$SIFTMAXIMAGESIZE.db --image_path $CWD/$INDIR --SiftExtraction.max_image_size $SIFTMAXIMAGESIZE";
print "$cmd\n";
system $cmd;
#
